...
import { FrappeContext, useFrappePostCall } from "frappe-react-sdk"
...
const BankEntryForm = ({ selectedTransaction }: { selectedTransaction: UnreconciledTransaction }) => {
    ...
    const { call: createBankEntry, loading, error } = useFrappePostCall<{ message: { transaction: BankTransaction, journal_entry: JournalEntry } }>('erpnext.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.create_bank_entry_and_reconcile')

    const setBankRecUnreconcileModalAtom = useSetAtom(bankRecUnreconcileModalAtom)
    const addToActionLog = useUpdateActionLog()

    const { file: frappeFile } = useContext(FrappeContext) as any
        ...
        createBankEntry({
            ...
            if (files.length > 0) {
                ...
                const uploadPromises = files.map(f => {
                    ...
                    }, (_bytesUploaded: any, _totalBytes: any, progress: any) => {
...
const Entries = ({ company, isWithdrawal, currency }: { company: string, isWithdrawal: boolean, currency: string }) => {
    ...
    const onAccountChange = (value: string, index: number) => {
        ...
        if (value) {
            const account = accounts?.find((acc: any) => acc.name === value)
