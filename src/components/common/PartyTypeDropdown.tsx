...
const PartyTypeDropdown = ({ value, onChange, readOnly, disabled, type, hideOptionsByType, valueProps, triggerProps, useInForm }: PartyTypeDropdownProps) => {
    ...
    const { data } = useFrappeGetDocList<PartyTypeOption[]>("Party Type", {
    ...
    const filteredData = useMemo(() => {
        ...
        let options: PartyTypeOption[] = data ?? [
        ...
        if (hideOptionsByType && type) {
            options = options.filter((option: PartyTypeOption) => option.account_type === type)
        }

        // Order by type if type is set
        if (type) {
            options = options.sort((a: PartyTypeOption) => a.account_type === type ? -1 : 1)
        }
    ...
    return (
        <Select onValueChange={onSelect} value={value} disabled={disabled}>
            ...
            <SelectContent>
                {filteredData.map((option: PartyTypeOption) => (
